...
export type MotionTextVariants = {
  char?: {
    [key: string]:
      | TargetAndTransition
      | ((custom: CharContext, current: Target, velocity: Target) => TargetAndTransition | string);
  };
  word?: {
    [key: string]:
      | TargetAndTransition
      | ((custom: WordContext, current: Target, velocity: Target) => TargetAndTransition | string);
  };
  line?: {
    [key: string]:
      | TargetAndTransition
      | ((custom: LineContext, current: Target, velocity: Target) => TargetAndTransition | string);
  };
};
...
export interface MotionTextProps extends Omit<HTMLMotionProps<"div">, "variants"> {
  text: string;
            ...
            <LineRenderer key={i}>
              ...
              >
                {line
                  ...
                  .filter((x) => WHITESPACE_REGEX.test(x))
                  .map((word, j, wordArr) => {
                    const WordRenderer = renderers?.word ?? DEFAULT_RENDERERS.word;
                      ...
                      <WordRenderer key={j}>
                        ...
                        >
                          {word.split("").map((char, k) => {
                            const CharRenderer = renderers?.char ?? DEFAULT_RENDERERS.char;
